...
namespace fc
{
   ...
   struct to_variant_help
   {
       static void to_variant(const Tuple &t, variants &vars)
       {
           vars[N - 1] = std::get<N - 1>(t);
           to_variant_help<decltype(t), N - 1>::to_variant(t, vars);
       };
   };

   template<class Tuple>
   struct to_variant_help<Tuple, 1>
   {
       static void to_variant(const Tuple &t, variants &vars)
       {
           vars[0] = std::get<0>(t);
   ...
   void to_variant(const std::tuple<Types...> &t, variant &v)
   {
       std::vector<variant> vars(sizeof...(Types));
       to_variant_help<decltype(t), sizeof...(Types)>::to_variant(t, vars);
       v = vars;
   }

   template<size_t I, class First, class... Types>
   struct from_variant_help
   {
       static auto from_variant(const variants &var) -> std::tuple<First, Types...>
       {
           return std::tuple_cat(std::make_tuple(var[I].as<First>( )), from_variant_help<I + 1, Types...>::from_variant(var));
       };
   };

   template<size_t I, class T1, class T2>
   struct from_variant_help<I, T1, T2>
   {
       static std::tuple<T1, T2> from_variant(const variants &var)
       {
           return std::make_tuple(var[I].as<T1>( ), var[I + 1].as<T2>( ));
       };
   };

   template<class... Types>
   void from_variant(const variant &v, std::tuple<Types...> &p)
   {
       p = from_variant_help<0, Types...>::from_variant(v.get_array( ));
   }
